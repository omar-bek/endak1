                                    ...
                                    <td>
                                        <div class="btn-group btn-group-sm" role="group">
                                            <a href="{{ route('admin.sub_categories.create') }}"
                                                class="btn btn-success mb-3">إضافة قسم فرعي</a>
                                            ...
                                            <form action="{{ route('admin.categories.toggle-status', $category->id) }}"
                                                ...
                                                @method('PATCH')
                                                <button type="submit"
                                                    class="btn btn-sm toggle-btn {{ $category->is_active ? 'btn-outline-danger' : 'btn-outline-success' }}"
                                            ...
                                            <a href="{{ route('admin.categories.fields.index', $category->id) }}"
                                                class="btn btn-sm btn-info">إدارة الحقول</a>
                                        </div>
                                    </td>
                                </tr>
                                @php
                                ...
                                @if ($subCategories->count() > 0)
                                    @foreach ($subCategories as $subCategory)
                                        <tr style="background:#f9f9f9;">
                                            ...
                                            <td style="padding-left:40px;">&raquo; {{ $subCategory->name_ar }}</td>
                                            <td style="padding-left:40px;">&raquo; {{ $subCategory->name_en ?? '-' }}</td>
                                            <td>
                                            ...
                                            <td>
                                                <div class="btn-group btn-group-sm" role="group">
                                                    ...
                                                    <a href="{{ route('admin.categories.fields.create', ['category' => $category->id, 'sub_category_id' => $subCategory->id]) }}"
                                                        class="btn btn-primary" title="إضافة حقل">
                                                        <i class="fas fa-plus"></i>
                                                    </a>

                                                    <!-- زر تفعيل/تعطيل للقسم الفرعي -->
                                                    <form
                                                        action="{{ route('admin.sub_categories.toggle-status', $subCategory->id) }}"
                                                        method="POST" style="display:inline-block">
                                                        @csrf
                                                        @method('PATCH')
                                                        <button type="submit"
                                                            class="btn toggle-btn {{ $subCategory->status == 'active' ? 'btn-outline-danger' : 'btn-outline-success' }}"
