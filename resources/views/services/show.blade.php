                ...
                <div class="card-body">
                    ...
                    <div class="d-flex justify-content-between align-items-start mb-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            <button type="button" class="btn btn-danger btn-sm" onclick="if(confirm('{{ __('messages.delete_confirm_prompt') }}')) { document.getElementById('delete-form').submit(); }">
                            ...
                            <div class="card-body p-0">
                                        ...
                                        <tbody>
                                            ...
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    ...
                                                    <img src="{{ asset('storage/' . (is_array($imagePath) ? $imagePath[0] : $imagePath)) }}" alt="{{ __('messages.image_alt') }}" class="img-thumbnail me-1 mb-1" style="width: 50px; height: 50px; object-fit: cover; border: 2px solid #009f9c;" data-bs-toggle="modal" data-bs-target="#imageModal" onclick="showImageModal('{{ asset('storage/' . (is_array($imagePath) ? $imagePath[0] : $imagePath)) }}')">
                                                    @endforeach
                                                    @else
                                ...
                                <div class="table-responsive">
                                    <table class="table mb-0 align-middle">
                                        <thead style="background-color: #007b7f; color: white;">
                                            <tr>
                                                <th class="text-center"><i class="fas fa-tag me-1 text-warning"></i> {{ __('messages.custom_field_type') }}</th>
                                                <th class="text-center"><i class="fas fa-info-circle me-1 text-warning"></i> {{ __('messages.custom_field_value') }}</th>
                                            </tr>
                                        </thead>
                                        <tbody>
    ...
    <div class="col-md-4">
        ...
        <div class="d-flex flex-column gap-2">
            ...
            <span class="badge bg-danger fs-6 p-2">
                <i class="fas fa-times-circle me-1"></i>{{ __('messages.status_unavailable') }}
            </span>
            @endif
        </div>
    </div>
...
<div class="col-lg-4">
    ...
    <div class="alert alert-warning mb-3">
        ...
        <strong>{{ __('messages.offer_rejected_title') }}</strong> {{ __('messages.offer_rejected_body') }}
    </div>
    @elseif($userOffer->is_expired)
    <div class="alert alert-danger mb-3">
        <i class="fas fa-clock"></i>
        <strong>{{ __('messages.offer_expired_title') }}</strong> {{ __('messages.offer_expired_body') }}
    </div>
    @elseif($userOffer->status === 'accepted')
    <div class="alert alert-success mb-3">
        <i class="fas fa-check-circle"></i>
        <strong>{{ __('messages.offer_accepted_title') }}</strong> {{ __('messages.offer_accepted_body') }}
    </div>
    @elseif($userOffer->status === 'pending')
    <div class="alert alert-info mb-3">
        <i class="fas fa-clock"></i>
        <strong>{{ __('messages.offer_pending_title') }}</strong> {{ __('messages.offer_pending_body') }}
    </div>
    @endif
    <div class="card mb-3 @if($userOffer->status === 'accepted') border-success @elseif($userOffer->status === 'rejected') border-danger @elseif($userOffer->is_expired) border-warning @else border-info @endif">
        ...
        <div class="card-body">
            <div class="row">
                <div class="col-md-6">
                    <h6 class="@if($userOffer->status === 'accepted') text-success @elseif($userOffer->status === 'rejected') text-danger @elseif($userOffer->is_expired) text-warning @else text-info @endif">{{ __('messages.offer_price') }}</h6>
            ...
            <div class="mt-2">
                ...
                <small class="text-danger">
                    <i class="fas fa-exclamation-triangle"></i> {{ __('messages.offer_expired_note') }}
                </small>
                @else
                <small class="text-muted">
                    <i class="fas fa-clock"></i> {{ __('messages.offer_expires_at') }} {{ $userOffer->expires_at->format('Y-m-d H:i') }}
                </small>
                @endif
            </div>
            @endif
            <div class="mt-2">
                <small class="text-muted">
                    <i class="fas fa-calendar"></i> {{ __('messages.offer_submitted_at') }} {{ $userOffer->created_at }}
                </small>
            </div>
    ...
    <div class="card mb-3 shadow-sm border-0" style="border-radius: 15px; overflow: hidden;">
        <div class="card-header text-white" style="background: linear-gradient(135deg, #007b8f, #00a6a6);">
            <h5 class="mb-0"><i class="fas fa-bolt me-2 text-warning"></i>{{ __('messages.quick_actions_title') }}</h5>
        </div>
        <div class="card-body bg-light">
            <div class="d-grid gap-2">
                ...
                @else
                <button class="btn btn-outline-secondary" disabled title="{{ __('messages.cannot_offer_tooltip') }}">
                ...
                @else
                <a href="{{ route('service-offers.index', $service) }}" class="btn text-white" style="background-color: #007b8f;">
                    <i class="fas fa-list"></i> {{ __('messages.view_offers_count_button') }} ({{ $service->pending_offers->count() }})
                </a>
...
<div class="card">
    <div class="card-header">
        <h5 class="mb-0">{{ __('messages.related_services_title') }}</h5>
    </div>
    <div class="card-body">
